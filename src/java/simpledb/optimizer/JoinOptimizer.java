...
public class JoinOptimizer {
    ...
    private static final int DEFAULT_BUFFER_SIZE = 4096*50;
    /**
     * Constructor
     *
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        int card = 1;
        int small_size=Integer.min(card1,card2);
        int big_size=Integer.max(card1,card2);
        if(joinOp==Op.EQUALS){
            if(t1pkey&&!t2pkey){
                card=card2;
        ...
        }else if(joinOp==Op.LIKE || joinOp==Op.NOT_EQUALS){
            card=card1*card2;
        }else{
    ...
    public List<LogicalJoinNode> orderJoins(
        ...
        int number_nodes=joins.size();
        PlanCache cache = new PlanCache();
        Set<LogicalJoinNode> sets = null;
        for(int size=1;size<=number_nodes;++size){
            for(Set<LogicalJoinNode> plan:this.enumerateSubsets(this.joins, size)){
                if(plan.size()==number_nodes){
                    sets=plan;
                }
                Double minimal_cost=Double.MAX_VALUE;
                CostCard best_plan=new CostCard();
                for(LogicalJoinNode p:plan){
                    CostCard cur_plan=computeCostAndCardOfSubplan(stats, filterSelectivities, p, plan, minimal_cost, cache);
                    if(cur_plan!=null){
                        best_plan=cur_plan;
                        minimal_cost=cur_plan.cost; 
                    }
                }
                if(best_plan.plan!=null){
                    cache.addPlan(plan, best_plan.cost,best_plan.card,best_plan.plan);
                }
            }   

        }
        return cache.getOrder(sets);
    }
